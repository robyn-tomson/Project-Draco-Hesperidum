...
def slots(screen):
    ...
    pilt1 = pygame.transform.scale(pygame.image.load("kirsid.jpg").convert(), (110, 120))
    pilt2 = pygame.transform.scale(pygame.image.load("Täringud.jpg").convert(), (110, 120))
    pilt3 = pygame.transform.scale(pygame.image.load("õnn.jpg").convert(), (110, 120))
    pilt4 = pygame.transform.scale(pygame.image.load("õnn.jpg").convert(), (110, 120))
    pildid = [pilt1, pilt2, pilt3, pilt4]

    for i in range(4):
        ruudud1.append(ruut(kohadx[0], kohady[i], 120, 110, white, nimed[i], pygame.font.Font('freesansbold.ttf', 40),
                            pildid[i], screen))
        ruudud2.append(ruut(kohadx[1], kohady[i], 120, 110, white, nimed[i], pygame.font.Font('freesansbold.ttf', 40),
                            pildid[i], screen))
        ruudud3.append(ruut(kohadx[2], kohady[i], 120, 110, white, nimed[i], pygame.font.Font('freesansbold.ttf', 40),
                            pildid[i], screen))
